...
namespace CatalogService.API.Controllers
{
    ...
    public class CatalogController : Controller
    {
        ...
        public CatalogController(CatalogContext context, IOptionsSnapshot<CatalogSettings> settigns)
        {
            _catalogContext = context ?? throw new ArgumentNullException(nameof(context));
            _catalogSettings = settigns.Value;

            context.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking;
        }

        //ProducesResponseType, bir Web API metodunun hangi HTTP durum kodlarını döndürebileceğini ve bu durum kodlarıyla birlikte dönebilecek içerik türünü(response type) belirtir.
        //Bu kisimlarin eklenmesi swagger tarafinda rahat bir dokumantasyon olusturmak icindir.
        [HttpGet]
        [Route("items")]
        ...
        public async Task<IActionResult> ItemAsync([FromQuery] int pageSize = 10, [FromQuery] int pageIndex = 0, string ids = null)
        {
            ...
            {
                var items = await GetItemsByIdsAnsync(ids);
                if (!items.Any())
                {
        ...
        private async Task<List<CatalogItem>> GetItemsByIdsAnsync(string ids)
        {
